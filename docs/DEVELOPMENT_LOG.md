...
## 2026-08-03 PostgreSQL 全量补数与源端边界审计

- 背景：Replay Key 更新后，需要把可取得的数据统一补到 2024-01-01 以来，并确认 15000 积分接口的真实归档范围和频率边界。
- 改动：新增每日指标、复权因子、涨跌停/炸板、连板天梯和市场涨跌家数/冰点表；补充可断点、受 90 次/分钟全局节流保护的并发回补命令。
- 数据：日线、个股资金流、板块资金流、每日指标、复权因子、涨跌停、连板和市场宽度均覆盖 2024-01-02 至 2026-07-31 的 624 个交易日，交易日缺口为 0。
- 数据：龙虎榜个股/席位覆盖 624 日；游资明细 166,243 行、624 日；机构调研 399,797 行、557 日成功。
- 源端边界：机构调研另有 67 日经三次请求仍返回 Replay 40203；券商月度金股仅 2026-07 有归档；东方财富人气有 51 个历史交易日无归档。上述日期均保留 `source_limited`/`unauthorized`/`unavailable` 证据，不伪造记录。
- 监控：系统状态 API 和页面新增源端受限计数，不再把“全部日期已检查”误报为“数据完整”；自动同步 timer 均处于 enabled/active。
- 验证：`66 passed` 单元测试；`python -m compileall -q quant_platform scripts`；`npm run build`；PostgreSQL 交易日集合差集审计；`git diff --check`。
- 后续：API 服务重启后发布新的状态语义；若要补齐机构调研 67 日和历史月度金股，必须更换为含对应历史归档/权限的数据源。
